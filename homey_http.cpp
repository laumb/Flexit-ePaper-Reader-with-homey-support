...
static String currentBaseUrl()
{
  ...
  if (g_data.ip.length() > 0)
  {
    return String("http://") + g_data.ip;
  }
...
static void handleAdmin()
{
  ...
  s += "<div class='actions'>";
  s += "<a class='btn secondary' href='/admin/export/homey'>Eksporter Homey-oppsett</a>";
  s += "<button class='btn secondary' type='button' onclick='shareHomeyExport()'>Del fil (mobil)</button>";
  s += "</div>";
  s += "<div class='help'>Mobil: del/send fil til e-post. Desktop: last ned fil og bruk den videre i Homey.</div>";
  s += "</div>";
  ...
  s += "<script>(function(){"
       "window.shareHomeyExport=async function(){"
       "try{"
       "const r=await fetch('/admin/export/homey',{credentials:'same-origin'});"
       "if(!r.ok) throw new Error('HTTP '+r.status);"
       "const blob=await r.blob();"
       "const file=new File([blob],'ventreader_homey_setup.json',{type:'application/json'});"
       "if(navigator.share && navigator.canShare && navigator.canShare({files:[file]})){"
       "await navigator.share({title:'VentReader Homey export',text:'VentReader Homey setup file',files:[file]});"
       "return;"
       "}"
       "const url=URL.createObjectURL(blob);"
       "const a=document.createElement('a');a.href=url;a.download='ventreader_homey_setup.json';document.body.appendChild(a);a.click();"
